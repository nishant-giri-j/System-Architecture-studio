...
# Fix 1
code = re.sub(r'return\s*\(\s*\{\/\* All Historical and Current Charts \*\/\}\s*<div', 
              r'return (<>{/* All Historical and Current Charts */}<div', 
              code)

# Fix 2
code = re.sub(r'\}\)\)\}\s*<\/div>\s*\);\s*\}\);',
              r'}))}</></div>);});',
              code)

# Fix 3
# bad_invocation regex
bad_inv_pattern = r'<div className="border-\[3px\] borde\{\/\* All Historical and Current Charts \*\/\}.*?<HistoryCharts.*?\/>\s*\)\}'
good_invocation = """<div className="border-[3px] border-[#161616] p-6 shadow-[4px_4px_0_#161616] bg-white flex flex-col mb-6">
                                      ...
                                      {proposedPlans.map((p, idx) => (
                                          <div 
                                              ...
                                              onClick={() => {
                                                  ...
                                                  const target = nodes.find((n: any) => n.id === p.targetNodeId);
                                                  if (target) {
                                          ...
                                          >
                                              <div className="flex gap-2">
                                                  <span className="bg-[#161616] text-[#ffde59] px-2 py-1 text-xs font-black uppercase">TARGET: {nodes.find((n: any) => n.id === p.targetNodeId)?.data.label || p.targetNodeId}</span>
                          ...
                          <HistoryCharts 
                              ...
                              nodes={nodes} 
                          />"""
code = re.sub(bad_inv_pattern, good_invocation, code, flags=re.DOTALL)
